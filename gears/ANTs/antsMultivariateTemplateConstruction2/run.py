...
Because antsMultivariateTemplateConstruction2 takes a variable number of files as inputs,
...
in the tag info file and pass them to antsMultivariateTemplateConstruction2. To avoid
...
container = '[matherlab/antsMultivariateTemplateConstruction2]'
...
def get_params():
    ...
    param_flags['image_summarization_statistic'] = '-a'
    param_flags['iteration_limit'] = '-i'
    param_flags['max_iterations'] = '-q'
    ...
    param_flags['registration_similarity_metric'] = '-m'
    param_flags['rigid_body_registration'] = '-r'
    param_flags['sharpening'] = '-A'
    param_flags['shrink_factors'] = '-f'
    param_flags['smoothing'] = '-s'
...
def get_command():
    """Builds the shell command that will run antsMultivariateTemplateConstruction2"""

    cmd = os.path.join(os.environ['ANTSPATH'], 'antsMultivariateTemplateConstruction2.sh')
    params = get_params()
